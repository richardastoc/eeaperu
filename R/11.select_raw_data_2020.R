...
#'
#' @examples
#' select_2020(eea_data_links_variables_df_manual,eea_raw_data)
select_2020=function(eea_data_links_variables_df_manual,
      ...
      for (j in 2:length(raw_data[[i]])) {
        ...
        if(dim(as.data.frame(raw_data[[i]][j]))[1]!=0){
          ...
          if(!is.null(unique(as.data.frame(raw_data[[i]][j])$CodCapitulo))){
            ...
            cap_activos=temp_raw_caps[i,6]
            cap_patrimo=temp_raw_caps[i,7]
            cap_ventas=temp_raw_caps[i,8]
            cap_insumo=temp_raw_caps[i,9]
            cap_taunet=temp_raw_caps[i,10]
            cap_acinta=temp_raw_caps[i,11]
            cap_public=temp_raw_caps[i,12]
            cap_result=temp_raw_caps[i,13]
            cap_ubruta=temp_raw_caps[i,14]
            cap_uopera=temp_raw_caps[i,15]
            cap_acfijo=temp_raw_caps[i,16]
            cap_empleo=temp_raw_caps[i,17]
            cap_salari=temp_raw_caps[i,18]

            pre_activos=str_c('dato',temp_raw_links[i,7])
            pre_patrimo=str_c('dato',temp_raw_links[i,10])
            pre_ventas=str_c('dato',temp_raw_links[i,13])
            pre_insumo=str_c('dato',temp_raw_links[i,16])
            pre_taunet=str_c('dato',temp_raw_links[i,19])
            pre_acinta=str_c('dato',temp_raw_links[i,22])
            pre_public=str_c('dato',temp_raw_links[i,25])
            pre_result=str_c('dato',temp_raw_links[i,28])
            pre_ubruta=str_c('dato',temp_raw_links[i,31])
            pre_uopera=str_c('dato',temp_raw_links[i,34])
            pre_acfijo=str_c('dato',temp_raw_links[i,37])
            pre_empleo=str_c('dato',temp_raw_links[i,40])
            pre_salari=str_c('dato',temp_raw_links[i,43])
            ...
            if(!is.na(cap_activos[[1]])){
              ...
              if(!is.na(unique(as.data.frame(raw_data[[i]][j])$CodCapitulo))){
                ...
                if(unique(as.data.frame(raw_data[[i]][j])$CodCapitulo)==cap_activos[[1]]){
                  ...
                  preguntas=as.data.frame(raw_data[[i]][j])[str_detect(names(as.data.frame(raw_data[[i]][j])),'dato|Clave|CLAVE|iruc|RUC')]
            ...
            if(!is.na(cap_ventas[[1]])){
              ...
              if(!is.na(unique(as.data.frame(raw_data[[i]][j])$CodCapitulo))){
                ...
                if(unique(as.data.frame(raw_data[[i]][j])$CodCapitulo)==cap_ventas[[1]]){
                  ...
                  pre_ventas1=str_c('RUC|iruc|Clave|CLAVE|',pre_ventas[[1]])

                  sub_set=preguntas[str_detect(names(preguntas),pre_ventas1)]

                  data_set_ventas=sub_set[which(sub_set$Clave==cla_ventass[[1]]),]%>%
                    select(-CLAVE)

                  names(data_set_ventas)=c('iruc','ventas')

                  datos_ventas[[i]]=data_set_ventas%>%
                    mutate(year=temp_raw_links[i,1][[1]],
                           sector=temp_raw_links[i,5][[1]],
...
# write_rds(datos_ventas1,'ventas_2020.rds')
# write_rds(datos_insumos1,'insumos_2020.rds')
# write_rds(datos_resultados1,'resultados_2020.rds')
# write_rds(datos_acfijo1,'activo_fijo_2020.rds')
# write_rds(datos_activos1,'activo_contable_2020.rds')
# write_rds(datos_impuesto1,'impuesto_2020.rds')
# write_rds(datos_intangibles1,'intangibles_2020.rds')
# write_rds(datos_publicidad1,'publicidad_2020.rds')
